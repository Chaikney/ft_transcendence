    ...
    if (gameId === 'sudoku') {
      try {
        ...
        if (newGame && newGame.id) {
          ...
          console.error("No se encontró el ID en la respuesta:", newGame);
        }
      } catch (err) {
        console.error("Failed to launch sudoku:", err);
      }
    } else {
        console.log("Chess route pending");
    }
  };
    ...
    <div className={styles.page}>
      ...
      {booted && (
        <div className={styles.cardsWrap + ' animate-fade-in'}>
          {GAMES.map((game) => (
            <div key={game.id} className={styles.card} onClick={() => handleGameSelect(game.id as 'chess' | 'sudoku')}>
