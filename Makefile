...
# copy_src: chess_src web_src ruby_src
stop:
...
db-build: secrets
	...
	$(BASECMD) build -f $(CONTHOME)/infra/srcs/reqs/postgres/Dockerfile --tag $(REPO)/db:0.1

# FIXME if we have the ruby version, it should be actually correct and not hardcoded...
# FIXME none of these separate build steps will work right now
ruby-build: secrets
...
# NOTE we need the app, config and bin directories, maybe not the others.
# This copies all into context for the Dockerfile to handle
ruby_src:
